...
def search_all_leiloes(keyword):
    ...
    # cur.execute("SELECT ndep, nome, local FROM dep where ndep = %s", (ndep,))
    cur.execute(
        "SELECT id_leilao,descricao from leiloes where produtos_ean=%s and terminado=false and cancelado=false  or descricao=%s and terminado=false and cancelado=false",
        (keyword, keyword))
    rows = cur.fetchall()
...
def get_leilao_user(current_user):
    ...
    # cur.execute("SELECT ndep, nome, local FROM dep where ndep = %s", (ndep,))
    cur.execute(
        "SELECT id_leilao,titulo,descricao,data_user_username,preco_corrente from leiloes where leiloes.data_user_username=%s UNION SELECT id_leilao,titulo,descricao,leiloes.data_user_username,preco_corrente from leiloes,licitacoes where leiloes_id_leilao=id_leilao and licitacoes.data_user_username=%s",
    ...
    for row in rows:
        logger.debug(row)
        content = {'id_leilão': int(row[0]), 'titulo': row[1], 'descrição': row[2], 'Leiloeiro': row[3],
                   'Valor do produto': row[4]}
        payload.append(content)  # appending to the payload to be returned
...
def add_leiloes(current_user):
    ...
    if rows_user[0][0] == False:
        ...
        values = (
        payload["titulo"], payload["descricao"], payload["detalhes"], payload["data_inicial"], payload["data_final"],
...
def update_leiloes(current_user, id):
    ...
    # cur.execute("SELECT ndep, nome, local FROM dep where ndep = %s", (ndep,))
    cur.execute(
        "SELECT titulo,descricao,detalhes from leiloes where leiloes.id_leilao=%s and data_user_username=%s and terminado=false and cancelado=false ",
        (id, current_user))
    row = cur.fetchall()
    if (cur.rowcount == 0):
        conn.close()
...
def write_licitacao_mural(texto, data, current_user, id):
    ...
    cur.execute(
        "SELECT distinct mensagem_mural.data_user_username from mensagem_mural where mensagem_mural.leiloes_id_leilao = %s UNION SELECT leiloes.data_user_username from leiloes where id_leilao = %s",
        (id, id))
    rows = cur.fetchall()
...
def get_licitacao(current_user, LeilaoID, licitacao):
    ...
    # cur.execute("SELECT ndep, nome, local FROM dep where ndep = %s", (ndep,))
    cur.execute(
        "SELECT preco_corrente,user_vencedor,terminado,cancelado,data_user_username from leiloes where leiloes.id_leilao=%s and terminado=false and data_inicial<%s and cancelado=false",
        (LeilaoID, data))
    rows = cur.fetchall()
    cur.execute("SELECT banido from data_user where username=%s",
                (current_user,))
...
def get_ban(current_user, user_name):
    ...
    if (len(rows) == 0):
        ...
        cur.execute(
            "SELECT id_leilao from leiloes where data_user_username=%s and terminado=false  and cancelado=false",
            (user_name,))
        rows_ban = cur.fetchall()
        for a in rows_ban:
            put_cancelar(a[0])
            cur.execute(
                "SELECT distinct mensagem_mural.data_user_username from mensagem_mural where mensagem_mural.leiloes_id_leilao = %s ",
                (a[0],))
        ...
        ####
        cur.execute(
            "SELECT licitacoes.leiloes_id_leilao from licitacoes,leiloes where licitacoes.data_user_username=%s and licitacoes.leiloes_id_leilao=id_leilao and cancelado=false and terminado=false",
            (user_name,))
        rows_leiloes = cur.fetchall()
        for c in rows_leiloes:
            cur.execute(
                "SELECT user_vencedor,preco_corrente,preco_inicial,data_user_username from leiloes where id_leilao=%s",
                (c[0],))
            rows_vencedor = cur.fetchall()
            if rows_vencedor[0][0] == user_name:
                cur.execute(
                    "SELECT licitacoes.valor,licitacoes.data_user_username from licitacoes,data_user where licitacoes.leiloes_id_leilao=%s and licitacoes.data_user_username=username and banido=false order by valor DESC",
                    (c[0],))
                ...
                cur.execute(
                    "SELECT valor from licitacoes where leiloes_id_leilao=%s and data_user_username=%s order by valor",
                    (c[0], user_name))
...
def get_top10(current_user):
    ...
    cur.execute(
        "Select user_vencedor,COUNT(*) from leiloes where terminado=true and cancelado=false and data_final_ > %s  and user_vencedor <>data_user_username group by user_vencedor order by count(*) desc Limit 10",
        (data,))
    rows_vencedores = cur.fetchall()
    cur.execute(
        "Select data_user_username,COUNT(*) from leiloes where terminado=true and cancelado=false and data_final_ > %s  group by data_user_username order by count(*) desc Limit 10",
        (data,))
